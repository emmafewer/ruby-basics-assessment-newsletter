...
def calculate_recipients
  ...
  new_array = SUBSCRIBERS.reject {|email| UNSUBSCRIBED.include? email}
end
...
def print_one_article(article)
  ...
  i = 0
  while i < article.length do 
    puts article[i][:title].to_s
    puts article[i][:author].to_s
    puts article[i][:text].to_s
    puts ""
    i +=1
  end
end
...
def print_many_articles(articles)
  ...
  #one_article = print_one_article(articles)
